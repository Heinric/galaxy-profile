...
def render(
    ...
    for arm_idx in range(arm_count):
        ...
        base_angle = arm_idx * angle_step + 25

        # Pequena variação sutil para evitar espirais idênticas
        dynamic_max_radius = MAX_RADIUS - (arm_idx * 6)
        dynamic_x_scale = X_SCALE + (arm_idx * 0.02)
        dynamic_y_scale = Y_SCALE + (arm_idx * 0.01)
